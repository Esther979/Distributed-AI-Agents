...
global skills: [fipa] {
    int N <- 8 min: 4 max: 20; //Number of queens
    ...
    init {
        ...
        create queen number: N { 
        	//Location
            my_col <- int(self); 
            location <- {my_col * (100.0 / N) + (100.0/N)/2, -10}; 
        }

        loop i from: 0 to: N - 1 {
            queen q <- queen[i];
            //predecessor & successor
            if (i > 0) { q.predecessor <- queen[i-1]; }
            if (i < N - 1) { q.successor <- queen[i+1]; }
        }

        do start_conversation to: [queen[0]] protocol: 'no-protocol' performative: 'inform' contents: ["start"];
    }
}
...
species queen skills: [fipa] {
    ...
    aspect default {
        if (my_row != -1) {
        	//red cycle of queen
            draw circle((100.0 / N) / 2.5) color: #red border: #black;
            draw string(my_col) color: #white size: 10 at: location - {2,0};
        }
    }
    ...
    action find_valid_position {
        if (!empty(untried_rows)) {
            ...
            if (is_safe(row_to_try, history_cache)) {
                my_row <- row_to_try;
                location <- {my_col * (100.0 / N) + (100.0/N)/2, my_row * (100.0 / N) + (100.0/N)/2};
                ...
                if (successor = nil) {
                    ...
                    // do start_conversation to: [self] protocol: 'no-protocol' performative: 'inform' contents: ["backtrack"];
                } else {
                    // Not the last Queen: send to next Queen(successor)
                    do start_conversation to: [successor] protocol: 'no-protocol' performative: 'inform' contents: ["find", new_history];
                }
            } else {
            ...
            // finish, feedback
            /*if untried_rows empty(Even after trying all the rows in the current column, it still didn't work):
            *Remove oneself from the chessboard.
            *If there is a predecessor, send a message asking the predecessor to "go back and re-examine".
            *If there is no previous one (and I am Queen 0), it means that all possible options have been exhausted and the search has been completely completed.
            */
            my_row <- -1; 
            location <- {my_col * (100.0 / N) + (100.0/N)/2, -10}; 
            ...
            if (predecessor != nil) {
                // nofity previous one
                do start_conversation to: [predecessor] protocol: 'no-protocol' performative: 'inform' contents: ["backtrack"];
            } else {
    ...
    bool is_safe (int try_row, list<int> history) {
        ...
        loop i from: 0 to: length(history) - 1 {
            ...
            if (try_row = other_row) { return false; }
            if (abs(my_col - other_col) = abs(try_row - other_row)) { return false; }
        }
        return true;
    }
}
